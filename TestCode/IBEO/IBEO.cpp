...
Network *connection;        // The connection to communication to the ibeo.
 ...
 */
bool IBEO::StartScanner(char * ip_addr, int port) {
 ...
 * Outputs: true if successful, false otherwise.
 */
bool IBEO::StartScanner() {
    return StartScanner((char *)IBEO_IP_ADDRESS, IBEO_PORT);
}
...
for(int i = 0; i < Lux->scan_data_header[Lux->curScanDataSource].scan_points; i++) {
	outfile << (int)Lux->scan_data_points[Lux->curScanDataSource][i].layer_echo << "," << (int)Lux->scan_data_points[Lux->curScanDataSource][i].flags << "," << Lux->scan_data_points[Lux->curScanDataSource][i].horiz_angle << "," << Lux->scan_data_points[Lux->curScanDataSource][i].radial_dist << "," << Lux->scan_data_points[Lux->curScanDataSource][i].echo_pulse_width << "," << Lux->scan_data_points[Lux->curScanDataSource][i].res << "\n";
}
outfile.close();
