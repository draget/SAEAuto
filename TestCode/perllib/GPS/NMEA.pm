...
#T. Drage - also return the speed in km/h
sub get_location_speed_time {
    #($latsign,$lat,$lonsign,$lon)
    my $self = shift;
...
sub get_fix_info {
    ...
    until ($self->parse eq 'GPGGA') {
	1;
    }
    ;				
    my $d = $self->{NMEADATA};
    return ($d->{fixq012}, $d->{num_sat_tracked}, $d->{hdop});
}
