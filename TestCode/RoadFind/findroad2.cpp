...
int main() {
	...
	while(i > 0 && i < xvalues.size()) {
		...
		if((! FoundStart) && InitialSearchDirn == 0) { // Initial search left
			...
			double cov = fabs(fit.getCovariance());

						std::cout << i << "," << xvalues[i] << "," << yvalues[i] << "," << pow(fit.getCoefficient(),2) << "\n";



			i++;

			if(cov > 10) { // We've reached the RHS edge
			...
			if(cov > 0.006 + covbase) {
				Slope = fit.getSlope(); Intercept = fit.getIntercept(); 
				break;			
			}
	...
	std::cout << "Edges are: " << *(xlast-1) << " " << *(xfirst-1) << " Slope: " << Slope << " Intercept: " << Intercept << '\n';
