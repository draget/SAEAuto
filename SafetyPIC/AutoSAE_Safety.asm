...
;AutoSAE_Safety.c,36 :: 		if(hb_interruptcount > 18) {
	MOVLW      128
	MOVWF      R0+0
	MOVLW      128
...
;AutoSAE_Safety.c,181 :: 		if(PORTB.B3 == 1) { arm_state = -1; UART1_Write_Text("AF 1\n"); }   // Arm button
	...
	CALL       _UART1_Write_Text+0
L_main57:
;AutoSAE_Safety.c,182 :: 		if(PORTB.B4 == 0) { arm_state = -1; PORTB.B0 = 1; UART1_Write_Text("AF 2\n"); }  // Throttle
	BTFSC      PORTB+0, 4
	GOTO       L_main58
	MOVLW      255
	MOVWF      _arm_state+0
...
;AutoSAE_Safety.c,190 :: 		else if(arm_state == 5 || arm_state == 7) {
	...
	GOTO       L_main64
L__main77:
;AutoSAE_Safety.c,191 :: 		if(PORTB.B4 == 0) { arm_state = -1; PORTB.B0 = 1; UART1_Write_Text("AF 3\n"); }      // Throttle
	BTFSC      PORTB+0, 4
	GOTO       L_main65
	MOVLW      255
	MOVWF      _arm_state+0
...
;AutoSAE_Safety.c,204 :: 		else if(arm_state == -1) {
	...
	BTFSS      STATUS+0, 2
	GOTO       L_main73
;AutoSAE_Safety.c,205 :: 		if(PORTB.B3 == 1) { PORTB.B6 = 0; arm_state = 0; }
	BTFSS      PORTB+0, 3
	GOTO       L_main74
	BCF        PORTB+0, 6
	CLRF       _arm_state+0
